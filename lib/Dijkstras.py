...
def dijkstra(graph, src, dest, visited=[], distances={}, predecessors={}):
    ...
    if src == dest:  # if source and destination are the same print out shorest path and exit
        ...
        while pred != None:  # create
            # append list path to show the prgevious predecessors
            path.append(pred)
            # get next predecessor and if none return none this breaks the next loop
        ...
        for neighbor in graph[src]:  # each neighbor for the new starting node
            if neighbor not in visited:  # if the neighbor hasnt been visited, check for new better paths
                # create new weight for this new node + weight of source node
                new_distance = distances[src] + graph[src][neighbor]
                # if new distance is less than the neightbor weight(if no weight assume infinity)
                if new_distance < distances.get(neighbor, float('inf')):
                    # set distances of this new neighboring node to the new distance
                    distances[neighbor] = new_distance
                    # set the predecessors of this new neighbor to the "current node"
        ...
        for k in graph:  # for each node
            if k not in visited:  # if node is not in visited,
                # add the weigths of every unvisited node
                unvisited[k] = distances.get(k, float('inf'))
        x = 0
        x = min(unvisited, key=unvisited.get)  # get the lowest weighted node
        # run dijkstra's algorithm on cheapest node
